...
# Type: New Service
#
# Title: M365 - Set Out of Office Reply
#
...
param (
    ...
    [parameter(Mandatory = $false)] 
    [String]$c_InternalText,

    [parameter(Mandatory = $false)] 
    [String]$c_ExternalText,
...
try {
    ...
    try {
        Write-au2matorLog -Type INFO -Text "Try to get UPN from Selected User"
        $UPN = (Get-ADUser -Identity $c_User).UserPrincipalName

        try {
            Write-au2matorLog -Type INFO -Text "Try to set OOF with MS Graph API"

            $URLSETOOF = "https://graph.microsoft.com/v1.0/users/$UPN/mailboxSettings" 
            if ($c_Type -eq "planned") {
                $BodyJsonSETOOF = @"
            {
                ...
                "automaticRepliesSetting": {
                    ...
                    "internalReplyMessage": "$c_InternalText",
                    "externalReplyMessage": "$c_ExternalText"
                }
            }
"@
            }
...
"@
            ...
            $au2matorReturn = "Out of Office Reply was set for User: $UPN"
            $AdditionalHTML = "Out of Office Reply was set for User: $UPN
            <br>
            "
            $Status = "COMPLETED"
        }
            ...
            Write-au2matorLog -Type ERROR -Text "Could not set OOF"
            Write-au2matorLog -Type ERROR -Text "Error: $Error"

            $au2matorReturn = "Could not set OOF, Error: $Error"
            $AdditionalHTML = "Could not set OOF
            <br>
            Error: $Error
            "
            $Status = "ERROR"
        ...
        Write-au2matorLog -Type ERROR -Text "Could not get UPN for the selected User"
        Write-au2matorLog -Type ERROR -Text "Error: $Error"

        $au2matorReturn = "Could not get UPN for the selected User, Error: $Error"
        $AdditionalHTML = "Could not get UPN for the selected User
        <br>
        Error: $Error
            "
        $Status = "ERROR"
